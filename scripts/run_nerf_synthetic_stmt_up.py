...
output_dir = "/cluster/scratch/jiezcao/jiameng/3dgs/benchmark_nerf_synthetic_stmt_up"
...
def train_scene(gpu, scene, factor):
    cmd = f"OMP_NUM_THREADS=4 CUDA_VISIBLE_DEVICES={gpu} python train.py -s {dataset_dir}/{scene} -m {output_dir}/{scene} -r {factor} --eval --white_background --port {6010 + int(gpu)}"
    print(cmd)
    if not dry_run:
        os.system(cmd)
