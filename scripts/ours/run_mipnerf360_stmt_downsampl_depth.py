...
def train_scene(gpu, scene, factor):
    ...
    trained_gaussian = os.path.join(get_folder, scene,
                                    "point_cloud/iteration_30000/point_cloud.ply")  # "./fused/"+scene+"_fused_x1.ply"
    for scale in [4, 2, 1]:
        ...
        cmd = f"python train_downsampl_depth.py -s {pseudo_gt} -m {model_path} -r 1 --port {3609 + int(gpu)} --load_gaussian {trained_gaussian} " \
              f"--H {H} --W {W}"
        print(cmd)
        if not dry_run:
            os.system(cmd)
