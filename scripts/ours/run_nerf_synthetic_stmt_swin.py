...
# output_dir = "/cluster/scratch/jiezcao/jiameng/3dgs/nerf_synthetic_ours_stmt_swin"# l1+ssim
output_dir = "/cluster/scratch/jiezcao/jiameng/3dgs/nerf_synthetic_ours_stmt_swin_l2" # l2
...
def train_scene(gpu, scene, factor):
    ...
    for scale in [4, 2, 1]:
        ...
        cmd = f"OMP_NUM_THREADS=4 CUDA_VISIBLE_DEVICES={gpu} python train_two_stage_syn.py -s {pseudo_gt} " \
              f"-m {model_path} -r 1 --white_background --port {4000 + int(gpu)} --load_gaussian {trained_gaussian}"
        print(cmd)
        if not dry_run:
            os.system(cmd)
